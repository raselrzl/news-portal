...
export function PrintNewsDetailsClient({
  ...
  const [currentTime, setCurrentTime] = useState<{
    ...
    hour: "০", minute: "০০", ampm: "এএম", weekday: "রবিবার",
    day: "০", month: "জানুয়ারি", year: "২০২৫",
    banglaDay: "০", banglaMonth: "বৈশাখ", banglaYear: "১৪৩২",
    hijriDay: "০", hijriMonth: "মুহররম", hijriYear: "১৪৪৬"
  });
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
              ...
              <a href="www.jagrotobarta.com" target="_blank" rel="noopener noreferrer" style={{ fontSize: "8px", color: "#00bfff", textDecoration: "underline", paddingLeft: "9px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: "6px", marginTop: "6px", flexWrap: "nowrap", fontSize: "10px" }}>
            <span style={{ background: "#ff4d4f", padding: "2px 6px", borderRadius: "6px" }}>
              {currentTime.day} {currentTime.month} {currentTime.year}
            </span>
            <span style={{ background: "#52c41a", padding: "2px 6px", borderRadius: "6px" }}>
              {currentTime.banglaDay} {currentTime.banglaMonth} {currentTime.banglaYear} বঙ্গাব্দ
            </span>
            <span style={{ background: "#1890ff", padding: "2px 6px", borderRadius: "6px" }}>
              {currentTime.hijriDay} {currentTime.hijriMonth} {currentTime.hijriYear} হিজরী
            </span>
          </div>

          <div style={{ fontStyle: "italic", marginTop: "4px", color: "#ffcc00", fontSize: "10px", textAlign: "right" }}>
            নিখুঁত খবর, আপনার বিশ্বাসে।
          </div>
        </header>

        <article style={{ flexGrow: 1, overflow: "auto", paddingBottom: "8px" }} className="border-1 p-4">
          <h1 style={{ fontSize: "20px", fontWeight: "bold", marginBottom: "8px", borderBottom: "1px solid gray" }}>
            {newsHeading}
          </h1>
          <div style={{ overflow: "hidden", marginBottom: "16px" }}>
            {newsPicture && (
              <img src={newsPicture} alt={newsPictureHeading || "News Image"} style={{ float: "left", width: "120px", height: "120px", objectFit: "cover", marginRight: "16px", marginBottom: "16px" }} />
            )}
            <p className="text-[12px] leading-[1.3] text-justify mb-4 whitespace-pre-line">{firstPart}</p>
            <div style={{ clear: "both", columnCount: 3, columnGap: "1rem", fontSize: "12px", lineHeight: 1.3, textAlign: "justify", whiteSpace: "pre-line" }}>
              {remainingPart}
            </div>
          </div>
        </article>
