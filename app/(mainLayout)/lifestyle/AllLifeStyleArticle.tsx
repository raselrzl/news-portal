...
async function getAllLifeStyleArticles(page: number = 1, pageSize: number = 8) {
  const skip = (page - 1) * pageSize;

  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "LIFESTYLE" },
      take: pageSize,
      skip: skip,
      select: {
        id: true,
...
export default async function AllLifeStyleArticles({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllLifeStyleArticles(currentPage);
