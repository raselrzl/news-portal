...
async function getAllTravelArticles(page: number = 1, pageSize: number = 8) {
...
export default async function AllTravelArticles({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllTravelArticles(currentPage);
