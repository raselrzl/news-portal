...
import AllFarmingArticles from "./AllFarmingArticle";
...
export default async function Farming({ searchParams }: SearchParamsProps) {
  const params = await searchParams;
  const currentPage = Number(params.page) || 1;
  await trackRoute("Farming");
      ...
      <div className="grid grid-cols-3 mt-10">
        <div className="col-span-3 md:col-span-1">
          <div className="font-extrabold pl-2 mb-2 flex items-center">
            <List className="h-5 w-5 mr-2" /> কৃষি উন্নয়ন
          </div>

          <div className="p-1 md:p-4">
            <Suspense key={currentPage} fallback={<LoadingSpinner />}>
              <AllFarmingArticles currentPage={currentPage} />
            </Suspense>
          </div>
        </div>
