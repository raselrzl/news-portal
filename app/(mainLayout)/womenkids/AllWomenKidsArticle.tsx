...
async function getAllWomenKidsArticles(page: number = 1, pageSize: number = 8) {
  const skip = (page - 1) * pageSize;

  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "WOMENKIDS" },
      take: pageSize,
      skip: skip,
      select: {
        id: true,
...
export default async function AllWomenKidsArticles({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllWomenKidsArticles(currentPage);
