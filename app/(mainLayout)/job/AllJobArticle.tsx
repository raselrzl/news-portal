...
async function getAllJobArticles(page: number = 1, pageSize: number = 8) {
  const skip = (page - 1) * pageSize;

  const [data, totalCount] = await Promise.all([
    prisma.newsArticle.findMany({
      where: { newsCategory: "JOB" },
      take: pageSize,
      skip: skip,
      select: {
        id: true,
...
export default async function AllJobArticles({
  currentPage,
}: {
  currentPage: number;
}) {
  const { articles, totalPages } = await getAllJobArticles(currentPage);
